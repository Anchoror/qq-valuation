...
function myAxios(axiosConfig, customOptions, loadingOptions) {
  ...
  let custom_options = Object.assign(
    {
      ...
      code_message_show: true // 是否开启status=false时的信息提示, 默认为false
    },
    customOptions
  )
  ...
  service.interceptors.response.use(
    (response) => {
      ...
      // 处理响应数据
      if (custom_options.code_message_show && response.data) {
        switch (response.data.code) {
          case 200:
            return custom_options.reduct_data_format ? response.data : response
          case 500:
            return custom_options.reduct_data_format ? response : response
          case 420:
            showFailToast({
